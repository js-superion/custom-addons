...
from openerp.addons.mail.models.mail_thread import MailThread as mail_thread
...
class mail_thread_super(models.AbstractModel):
    ...
    def message_post(self, body='', subject=None, message_type='notification',
        ...
        if context.get('mail_post_autofollow') and self.ids and partner_ids:
        ...
        if (context.get('mail_save_message_last_post') and
            ...
            if not subtype_rec.internal:
                # done with SUPERUSER_ID, becacontextuse on some models users can post only with read access, not necessarily write access
                self.sudo().write({'message_last_post': fields.Datetime.now()})
        if new_message.author_id and model and self.ids and message_type != 'notification' and not context.get('mail_create_nosubscribe'):
